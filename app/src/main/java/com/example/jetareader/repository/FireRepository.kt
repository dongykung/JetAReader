...
class FireRepository @Inject constructor(private val queryBook: Query) {
    ...
    suspend fun getAllMyBooksFromDatabase(): DataOrException<List<MBook>, Boolean, Exception> {
        val dataOrException = DataOrException<List<MBook>, Boolean, Exception>()
        try {
            dataOrException.loading = true
            dataOrException.data =
                queryBook.whereEqualTo("user_id", FirebaseAuth.getInstance().currentUser?.uid).get()
